#!/usr/bin/env python3
"""Official scorer. This is the exact program we run; nothing about the metric is a surprise.

    python scripts/grader.py --pred results/predictions.jsonl --gold data/dev.jsonl \
        [--noisy data/dev_noisy.jsonl] [--hostile data/hostile.jsonl] \
        [--schema data/schema.json] [--report results/report.json] [--quiet]
...
#: `scripts/baseline.py`, trained on data/train.jsonl.gz and scored on the development files, gets:
#:
...
def schema_valid(pred: dict, schema: dict | None) -> bool:
    ...
    if schema is None:
        schema = json.loads(
            (Path(__file__).resolve().parent.parent / "data" / "schema.json").read_text()
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.quiet:
        ...
        print(f"Inbox triage grader {GRADER_VERSION}")
